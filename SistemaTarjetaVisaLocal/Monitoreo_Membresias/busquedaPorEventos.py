...
chrome_options.set_capability('se:name','Prueba Busqueda Por Eventos Membresia' )
...
class busquedaPorEventos(BaseCase):
    def runTest(self):  
        try:
            ...
            sleep(2)
            driver.find_element(by= By.ID, value='frmMonitoreoSolicitudes:selEvento_label').click()
        
            sleep(2)
            driver.find_element(by= By.ID, value='frmMonitoreoSolicitudes:selEvento_15').click()
        
            sleep(2)
            fechaInicialEvento = datetime.now() - timedelta(days=60)
            fechaInicialEvento = fechaInicialEvento.strftime('%d/%m/%Y')
            driver.find_element(by= By.ID, value='frmMonitoreoSolicitudes:calFechaInicial_input').send_keys(fechaInicialEvento)
        
            sleep(2)
            fechaFinalEvento = datetime.now().strftime('%d/%m/%Y')
            driver.find_element(by= By.ID, value='frmMonitoreoSolicitudes:calFechaFinal_input').send_keys(fechaFinalEvento)
            ...
            sleep(5)
            driver.save_screenshot('Prueba_Exitosa_BusquedaMembresiaPorEventos.png')
            ...
            sleep(2)
            driver.save_screenshot('Prueba_Fallida_BusquedaMembresiaPorEventos.png')
