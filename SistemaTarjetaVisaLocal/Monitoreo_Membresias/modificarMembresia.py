...
class modificacionMembresia(BaseCase):
    def runTest(self):  
        try:
            ...
            sleep(2)
            driver.find_element(by= By.ID, value='frmModificarSolicitud:tabviewInfoAdicional:rDetallesMembresia:0:txtNombre').clear()
        
            sleep(2)
            driver.find_element(by= By.ID, value='frmModificarSolicitud:tabviewInfoAdicional:rDetallesMembresia:0:txtNombre').send_keys('PRUEBA MODIFICACION DATOS SELENIUM')
        
            sleep(2)
            driver.find_element(by= By.ID, value='frmModificarSolicitud:btnValidarSolicitudMembresia').click()
        
            sleep(2)
            driver.find_element(by= By.ID, value='frmConfirmarDatos:btnModificarSolicitudMembresia').click()


            #Se guarda un screenshot al terminar la prueba
            sleep(5)
            driver.save_screenshot('Prueba_Exitosa_ModificacionMembresia.png')
            ...
            sleep(2)
            driver.save_screenshot('Prueba_Fallida_ModificacionMembresia.png')
