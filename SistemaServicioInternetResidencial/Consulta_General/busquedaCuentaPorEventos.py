...
chrome_options.set_capability('se:name','Prueba Busqueda Cuenta Por Eventos' )
...
class BusquedaCuentaPorEventos(BaseCase):
    def runTest(self):  
        try:
            ...
            sleep(2)
            driver.find_element(by= By.ID, value='frmBusquedaGeneral:sEventos').click()

            sleep(2)
            driver.find_element(by= By.ID, value='frmBusquedaGeneral:sEventos_1').click()
        
            sleep(2)
            fechaInicial = datetime.now() - timedelta(days=60)
            fechaInicial = fechaInicial.strftime('%d/%m/%Y')
            driver.find_element(by= By.ID, value='frmBusquedaGeneral:dFechaInicial_input').send_keys(fechaInicial)
        
            sleep(2)
            fechaFinal = datetime.now().strftime('%d/%m/%Y')
            driver.find_element(by= By.ID, value='frmBusquedaGeneral:dFechaFinal_input').send_keys(fechaFinal)
            ...
            sleep(12)
            driver.save_screenshot('Prueba_Exitosa_BusquedaCuentaPorEventos.png')
            ...
            sleep(2)
            driver.save_screenshot('Prueba_Fallida_BusquedaCuentaPorEventos.png')
