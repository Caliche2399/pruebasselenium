...
chrome_options.set_capability('se:name','Prueba Busqueda Solicitud Por Eventos' )
...
class BusquedaSolicitudPorEventos(BaseCase):
    def runTest(self):  
        try:
            ...
            sleep(2)
            driver.find_element(by= By.ID, value='frmMonitoreoSolicitudes:selEvento').click()

            sleep(2)
            driver.find_element(by= By.ID, value='frmMonitoreoSolicitudes:selEvento_2').click()
        
            sleep(2)
            fechaInicial = datetime.now() - timedelta(days=60)
            fechaInicial = fechaInicial.strftime('%d/%m/%Y')
            driver.find_element(by= By.ID, value='frmMonitoreoSolicitudes:calFechaInicial_input').send_keys(fechaInicial)
        
            sleep(2)
            fechaFinal = datetime.now().strftime('%d/%m/%Y')
            driver.find_element(by= By.ID, value='frmMonitoreoSolicitudes:calFechaFinal_input').send_keys(fechaFinal)
            ...
            sleep(12)
            driver.save_screenshot('Prueba_Exitosa_BusquedaSolicitudPorEventos.png')
            ...
            sleep(2)
            driver.save_screenshot('Prueba_Fallida_BusquedaSolicitudPorEventos.png')
