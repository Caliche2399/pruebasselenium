...
chrome_options.set_capability('se:name','Prueba Ingreso Evento Solicitud' )
...
class IngresoEventoSolicitud(BaseCase):
    def runTest(self):  
        try:
            ...
            sleep(2)
            driver.find_element(by= By.ID, value='frmMonitoreoSolicitudes:txtNoSolicitud').send_keys(NUMERO_SOLICITUD)

            sleep(2)
            driver.find_element(by= By.ID, value='frmMonitoreoSolicitudes:btnBuscar').click()

            sleep(2)
            driver.find_element(by= By.ID, value='frmMonitoreoSolicitudes:tvResultados:tblResultados:mbtnEventos_button').click()

            listadoEventos = driver.find_elements(by= By.TAG_NAME, value="li")

            for opcionEvento in listadoEventos:
                if(opcionEvento.text == "SOLICITUD PENDIENTE DE INSTALACION"):
                    opcionEvento.click() 

            sleep(2)
            driver.find_element(by= By.ID, value='frmProcesarCambioEstado:txtObservaciones').send_keys('PRUEBA INGRESO EVENTO SOLICITUD')

            sleep(2)
            driver.find_element(by= By.ID, value='frmProcesarCambioEstado:btnConfirmar').click()


            #Se guarda un screenshot al terminar la prueba
            sleep(5)
            driver.save_screenshot('Prueba_Exitosa_IngresoEventoSolicitud.png')
            ...
            sleep(2)
            driver.save_screenshot('Prueba_Fallida_IngresoEventoSolicitud.png')
...
def seleccionarAplicacionMenu():
    ...
    try:
        ...
        for aplicacionMenu in listadoMenu:
            if(aplicacionMenu.text == "MONITOREO DE SOLICITUDES"):
