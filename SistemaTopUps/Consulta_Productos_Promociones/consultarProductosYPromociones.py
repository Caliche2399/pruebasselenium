...
class consultarProductosYPromociones(BaseCase):
    def runTest(self):  
        try:
            ...
            sleep(0.5)
            driver.find_element(by= By.ID, value='frmConsultaProductos:selOneMenuPais').click()

            sleep(2)
            driver.find_element(by= By.ID, value='frmConsultaProductos:selOneMenuPais_2').click()

            sleep(5)
            driver.find_element(by= By.ID, value='frmConsultaProductos:selOneMenuOperador').click()

            sleep(2)
            driver.find_element(by= By.ID, value='frmConsultaProductos:selOneMenuOperador_2').click()

            sleep(2)
            driver.find_element(by= By.ID, value='frmConsultaProductos:selOneMenuCategoria').click()

            sleep(2)
            driver.find_element(by= By.ID, value='frmConsultaProductos:selOneMenuCategoria_1').click()

            sleep(2)
            driver.find_element(by= By.ID, value='frmConsultaProductos:btnConsultarProductos').click()

            #Se guarda un screenshot al terminar la prueba
            sleep(2)
            driver.save_screenshot('Prueba_Exitosa_ConsultaProductosPromociones.png')
            ...
            sleep(2)
            driver.save_screenshot('Prueba_Fallida_ConsultaProductosPromociones.png')
...
def seleccionarAplicacionMenu():
    ...
    try:
        ...
        for aplicacionMenu in listadoMenu:
            if(aplicacionMenu.text == "CONSULTA DE PRODUCTOS Y PROMOCIONES"):
