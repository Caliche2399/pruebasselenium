...
chrome_options.set_capability('se:name','Prueba Generacion Reporte Pagos Tarjeta Visa Excel' )
...
class reportePagosTarjetaVisaExcel(BaseCase):
    def runTest(self):  
        try:
            ...
            sleep(2)
            driver.find_element(by= By.ID, value='frmReportes:selReportes_3').click()
        
            sleep(2)
            fechaInicialReporte = datetime.now() - timedelta(days=60)
            fechaInicialReporte = fechaInicialReporte.strftime('%d/%m/%Y')
            driver.find_element(by= By.ID, value='formaDatos:fechaInicial_input').send_keys(fechaInicialReporte)
        
            sleep(2)
            fechaFinalReporte = datetime.now().strftime('%d/%m/%Y')
            driver.find_element(by= By.ID, value='formaDatos:fechaFinal_input').click(fechaFinalReporte)
        
            sleep(2)
            driver.find_element(by= By.CSS_SELECTOR, value='#formaDatos\:sorFormato > tbody:nth-child(1) > tr:nth-child(2) > td:nth-child(1) > div:nth-child(1) > div:nth-child(2)').click()
            ...
            sleep(15)
            driver.save_screenshot('Prueba_Exitosa_ReportePagosTarjetaVisaExcel.png')
            ...
            sleep(2)
            driver.save_screenshot('Prueba_Fallida_ReportePagosTarjetaVisaExcel.png')
