...
chrome_options.set_capability('se:name','Prueba Impresion De Recibo' )
...
class impresionDeRecibo(BaseCase):
    def runTest(self):  
        try:
            ...
            sleep(2)
            driver.find_element(by= By.ID, value='frmImpresionRecibo:itSerieFactura').send_keys(SERIE)
        
            sleep(2)
            driver.find_element(by= By.ID, value='frmImpresionRecibo:inNumeroFactura_input').send_keys(NUMERO)
        
            sleep(2)
            driver.find_element(by= By.ID, value='frmImpresionRecibo:pnlDatosFactura_content').click()
        
            sleep(2)
            driver.find_element(by= By.ID, value='frmImpresionRecibo:btnGenerarRecibo').click()


            #Se guarda un screenshot al terminar la prueba
            sleep(5)
            driver.save_screenshot('Prueba_Exitosa_ImpresionRecibo.png')
            ...
            sleep(2)
            cerrarSesion()
            driver.save_screenshot('Prueba_Fallida_ImpresionRecibo.png')
...
def seleccionarAplicacionMenu():
    ...
    try:
        ...
        for aplicacionMenu in listadoMenu:
            if(aplicacionMenu.text == "IMPRESION DE RECIBOS"):
