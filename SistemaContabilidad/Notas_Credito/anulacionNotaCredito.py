...
chrome_options.set_capability('se:name','Prueba Anulacion Nota Credito' )
...
class anulacionNotaCredito(BaseCase):
    def runTest(self):  
        try:
            ...
            sleep(2)
            driver.find_element(by= By.ID, value='tabViewPrincipal:frmAnulaNotaCredito:itSerienc').send_keys(SERIE)
        
            sleep(2)
            driver.find_element(by= By.ID, value='tabViewPrincipal:frmAnulaNotaCredito:itNumeronc').send_keys(NUMERO)
        
            sleep(2)
            driver.find_element(by= By.ID, value='tabViewPrincipal:frmAnulaNotaCredito:panelBusqueda_content').click()
        
            sleep(2)
            driver.find_element(by= By.ID, value='tabViewPrincipal:frmAnulaNotaCredito:btnAnularNotaCredito').click()
        
            sleep(2)
            driver.find_element(by= By.ID, value='frmConfirmacionAnulacion:itRazonAnulacion').send_keys('PRUEBA ANULACION NOTA CREDITO SELENIUM')
        
            sleep(2)
            driver.find_element(by= By.ID, value='frmConfirmacionAnulacion:cmdProcesarAnulacion').click()


            #Se guarda un screenshot al terminar la prueba
            sleep(8)
            driver.save_screenshot('Prueba_Exitosa_AnulacionNotaDeCredito.png')
            ...
            sleep(2)
            driver.save_screenshot('Prueba_Fallida_AnulacionNotaDeCredito.png')
