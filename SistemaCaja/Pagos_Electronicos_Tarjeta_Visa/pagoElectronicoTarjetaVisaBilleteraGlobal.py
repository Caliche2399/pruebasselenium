...
TERMINAL_ID = 'TESTSELENIUM2'
TNSID_TXID = 'TESTSELENIUM23'

#Configuración del driver
chrome_options = webdriver.ChromeOptions()
chrome_options.set_capability('se:name','Prueba Pago Electronico Tarjeta Visa Billetera Global' )
...
class PagoElectronicoTarjetaVisaBilleteraGlobal(BaseCase):
    def runTest(self):  
        try:
            ...
            sleep(2)
            driver.find_element(by= By.ID, value='frmBusquedaMembresia:numeroCuentaBusqueda_input').send_keys(NUMERO_MEMBRESIA)

            sleep(2)
            driver.find_element(by= By.ID, value='frmBusquedaMembresia:btnBuscar').click()

            sleep(2)
            driver.find_element(by= By.ID, value='frmDatosPago:tiposPago').click()

            sleep(2)
            driver.find_element(by= By.ID, value='frmDatosPago:tiposPago_4').click()

            sleep(2)
            driver.find_element(by= By.ID, value='frmDatosPago:terminalId').send_keys(TERMINAL_ID)

            sleep(2)
            driver.find_element(by= By.ID, value='frmDatosPago:tnsidTxid').send_keys(TNSID_TXID)

            sleep(2)
            driver.find_element(by= By.ID, value='frmDatosPago:observaciones').send_keys('OBSERVACIONES PAGO ELECTRONICO TARJETA VISA SELENIUM')

            sleep(2)
            driver.find_element(by= By.ID, value='frmDatosPago:correlTransBilletera_input').send_keys(TRANSACCION_BILLETERA)

            sleep(2)
            driver.find_element(by= By.ID, value='frmDatosPago:btnAplicar').click()

            sleep(2)
            driver.find_element(by= By.ID, value='frmConfirmacion:cmdAplicarPago').click()


            #Se guarda un screenshot al terminar la prueba
            sleep(5)
            driver.save_screenshot('Prueba_Exitosa_PagoElectronicoTarjetaVisaBilleteraGlobal.png')
            ...
            sleep(2)
            driver.save_screenshot('Prueba_Fallida_PagoElectronicoTarjetaVisaBilleteraGlobal.png')
...
def seleccionarAplicacionMenu():
    ...
    try:
        ...
        for aplicacionMenu in listadoMenu:
            if(aplicacionMenu.text == "PAGOS ELECTRONICOS TARJETA VISA LOCAL"):
