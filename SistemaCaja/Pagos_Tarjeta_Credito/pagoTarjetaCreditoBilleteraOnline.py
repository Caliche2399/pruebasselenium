...
chrome_options.set_capability('se:name','Prueba Pago Tarjeta Credito Billetera Online' )
...
class PagoTarjetaCreditoBilleteraOnline(BaseCase):
    def runTest(self):  
        try:
            ...
            for opcion in listadoTabla:
                if(opcion.text == NUMERO_CUENTA):
                    opcion.click()

            sleep(8)
            driver.find_element(by= By.CSS_SELECTOR, value='#frmMetodoPagoOnline\:btCrearPerfilNuevo').click()
            ...
            sleep(2)
            driver.find_element(by= By.ID, value='frmDatosFacturacion:itObservaciones').send_keys('OBSERVACIONES PAGO ONLINE BILLETERA')
            ...
            sleep(5)
            driver.save_screenshot('Prueba_Exitosa_PagoTarjetaCreditoBilleteraOnline.png')
            ...
            sleep(2)
            driver.save_screenshot('Prueba_Fallida_PagoTarjetaCreditoBilleteraOnline.png')
