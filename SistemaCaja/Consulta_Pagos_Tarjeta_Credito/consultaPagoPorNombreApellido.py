...
chrome_options.set_capability('se:name','Prueba Consulta Pago Tarjeta Credito Por Nombre Apellido' )
...
class ConsultaPagoTarjetaCreditoPorNombreApellido(BaseCase):
    def runTest(self):  
        try:
            ...
            sleep(2)
            driver.find_element(by= By.ID, value='frmBusquedaPagos:nombre').send_keys(NOMBRE)
        
            sleep(2)
            driver.find_element(by= By.ID, value='frmBusquedaPagos:apellido').send_keys(APELLIDO)
        
            sleep(2)
            fechaInicial = datetime.now() - timedelta(days=60)
            fechaInicial = fechaInicial.strftime('%d/%m/%Y')
            driver.find_element(by= By.ID, value='frmBusquedaPagos:fechaInicial_input').send_keys(fechaInicial)
        
            sleep(2)
            fechaFinal = datetime.now().strftime('%d/%m/%Y')
            driver.find_element(by= By.ID, value='frmBusquedaPagos:fechaFinal_input').send_keys(fechaFinal)
            ...
            sleep(5)
            driver.save_screenshot('Prueba_Exitosa_ConsultaPagoTarjetaCreditoPorNombreApellido.png')
            ...
            sleep(2)
            driver.save_screenshot('Prueba_Fallida_ConsultaPagoTarjetaCreditoPorNombreApellido.png')
